...
class MultiSerializerMixin:
    ...
    def paginator(self):
        """
        Look for paginator class in self.paginator_action_classes, which
        should be a dict mapping action name (key) to serializer class (value),
        i.e.:

        class MyViewSet(MultiSerializerViewSetMixin, ViewSet):
            paginator_class = MyDefaultPaginator
            paginator_action_classes = {
               'list': MyListPaginator,
               'my_action': MyActionPaginator,
            }
        ...
        If there's no entry for that action then just fallback to the regular
        paginator lookup: self.paginator property, DefaultPaginator.
        """
        try:
            if not hasattr(self, "_paginator"):
                self._paginator = self.paginator_action_classes[self.action]()
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            self.kwargs[lookup_url_kwarg] = (
                response.data.get(self.lookup_field) or response.data["id"]
            )
        except KeyError:
