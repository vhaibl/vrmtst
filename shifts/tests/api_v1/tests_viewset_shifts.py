...
def test_get_shift_for_employee(tz, employee_api, make_shift):
    shift = make_shift(organization=employee_api.employee.organization)
    assert employee_api.get(f"/api/v1/shifts/{shift.id}/book/") == [
        {
            "id": shift.id,
            "start": shift.start.astimezone(tz).isoformat(),
            "end": shift.end.astimezone(tz).isoformat(),
...
def test_get_shift_for_employee(tz, basic_api, employee, make_shift):
